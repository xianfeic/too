...
echo 'alias too="$HOME/core-bridgechain/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-bridgechain"
git clone "https://github.com/xianfeic/too" "$HOME/core-bridgechain" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/xianfeic/too'"
...
rm -rf "$HOME/.config/@too"
rm -rf "$HOME/.config/@too"
rm -rf "$HOME/.config/too-core"
